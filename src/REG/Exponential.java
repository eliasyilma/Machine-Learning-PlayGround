...
public class Exponential {
    ...
    public Exponential(LinkedList<Point2> dta) {
        data = dta;
    }
    ...
    public float y_estimate(float x_val) {
        return (float) Math.exp(slope * (x_val) + (intercept));
    }

    public float x_estimate(float y_val) {
        return (float) ((Math.log(y_val) - (intercept))/slope);
    }
}
