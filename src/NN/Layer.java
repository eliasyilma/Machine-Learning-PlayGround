...
public class Layer {
    ...
    public void draw_plus_btn(Graphics2D g2d) {
        ...
        pos_btn_x=x;
        pos_btn_y=y;
        if (plus_btn_status == 2) {
            g2d.setColor(btn_clicked_color);
        } else if (plus_btn_status == 1) {
            g2d.setColor(btn_entered_color);
        } else {
    ...
    public boolean pos_btn_clicked(int mx,int my){
        if(Math.sqrt(Math.pow(mx-pos_btn_x,2)+Math.pow(my-pos_btn_y,2))<30.0f)
            return true;
        return false;
    }

    public boolean neg_btn_clicked(int mx,int my){
        if(Math.sqrt(Math.pow(mx-neg_btn_x,2)+Math.pow(my-neg_btn_y,2))<30.0f)
            return true;
        return false;
    }    
